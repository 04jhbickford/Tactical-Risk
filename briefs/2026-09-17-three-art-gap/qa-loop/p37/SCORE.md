# p37 SCORE — V2.81.52-three-polish.37
...
**Albedo rev:** `p37b` (Cape L-band killed; family unchanged).
...
1. **Lock plate** — Cursor GenerateImage (Tesla off) painted the Oceania
   watercolor parchment lock plus Europe/Africa, Asia, world, ocean-wash,
   and a full Africa continent plate into `refs/p37-gen/`. Not a copyright scan.
...
3. **Bind** — `loadWorldLandAlbedo` loads `world-land-albedo.png?v=p37b` and
...
5. **p37b Cape** — dest y=1680 on the Europe/Africa theater cut a hard
   L-band across South Africa (bbox y=1586–1923) and also mapped the world
   plate's bottom vignette onto the Cape. Theater is northern names only;
   a continuous Africa continent plate pastes through the full AF mask with
   dest covering the Cape. `flatten_region_luma` + `heal_horiz_luma_step`.
...
| Even lighting (no Africa blotches) | **PASS** | `africa-even.png` (p37b): full southern Africa in frame. No hard horizontal L-band across the Cape. Continuous ochre parchment. Africa blotch gone. |
| Albedo bound | **PASS** | `painted albedo bound 4096 2340 …?v=p37b`. Inspect `albedoBound: true`, `stainFallback: false`, `albedoRev: p37b`, `watercolorParchment`, `styleRef: oceania-watercolor`. |
...
inspect:      paintedAlbedo · albedoBound · stainFallback=false · 4096×2340
              albedoRev=p37b · watercolorParchment · styleRef=oceania-watercolor
...
- `africa-even.png` — p37b Cape, no hard horiz band
- `sea-zones-ink.png` / `med-no-clip.png` / `unit-bg-unified.png`
...
- `vercel-live-mid-390.png` — live preview `?three=1` = `V2.81.52-three-polish.37`
- `vercel-live-inspect.json` — `albedoBound` · `albedoRev=p37b` · `styleRef=oceania-watercolor`

**Vercel:** https://tactical-risk20-97h16q7kz-james-projects-20d8de40.vercel.app/?three=1  
