...
2. **Bake** — `tools/bake-world-land-albedo.py` adds coastal/lowland sage
   under parchment, Imhof inked peaks + short hatch/soft AO (not GIS DEM),
   Risk continent washes ≤22% multiply (AA-PALETTE / AA-RISK-HOMAGE),
   destippled parchment-sea tile. Mask-only joins held. Cape L-band held.
3. **War overlay** — sea-lane dashes killed. Land ink 2.6/0.86 matches sea
   ...
   tan `#8E6A38` + dark rim vs parchment.
...
| 8 Units | **PASS** | `near-opaque-plastic.png` / `tan-units-contrast.png`: solid molded plastic, contact shadow, no parchment see-through. Tan rim vs paper. `opaquePlastic: true`. |
