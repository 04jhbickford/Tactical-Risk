#!/usr/bin/env node
// Capture 390 stills for polish.39. Requires a local or remote ?three=1 URL.
// Usage: node tools/capture-three-p39.mjs [baseUrl]
...
const outDir = join(root, 'briefs/2026-09-17-three-art-gap/qa-loop/p39');
...
async function serveLocal() {
  ...
  return { server, url: 'http://127.0.0.1:4183/index.html?three=1' };
}
...
d.rectangle((left.width + 8, 8, left.width + 180, 36), fill=(24, 26, 24))
...
async function main() {
  ...
  await shot('land-borders-clear.png');
  ...
  await shot('aus-borders-fit.png');
  ...
  const styleRef = join(root, 'briefs/2026-09-17-three-art-gap/refs/p39-gen/style-ref-oceania-beautiful.png');
  composePair(
