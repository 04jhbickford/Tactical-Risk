#!/usr/bin/env node
// Capture 390 stills for polish.40. Requires a local or remote ?three=1 URL.
// Usage: node tools/capture-three-p40.mjs [baseUrl]
...
const outDir = join(root, 'briefs/2026-09-17-three-art-gap/qa-loop/p40');
...
async function serveLocal() {
  ...
  return { server, url: 'http://127.0.0.1:4184/index.html?three=1' };
}
...
`;
  ...
  const styleRef = join(root, 'briefs/2026-09-17-three-art-gap/refs/p40-gen/style-ref-oceania-beautiful.png');
  composePair(
  ...
  const p39mid = join(root, 'briefs/2026-09-17-three-art-gap/qa-loop/p39/europe-mid-390.png');
  composePair(
    join(outDir, 'europe-mid-390.png'),
    p39mid,
    join(outDir, 'mid-vs-p39.png'),
    'p40 mid',
    'p39b mid',
  );
}
